...
def validate(**data_dict):
    ...
    max_cameras_in_installation_channels_all = (
        data_dict["no_of_transmitters"] * data_dict["no_of_channels"]
    )
    max_cameras_in_installation_channels_ce = (
        data_dict["no_of_transmitters"] * data_dict["no_of_ce_channels"]
    )
    ...
    if no_of_cameras > max_cameras_in_installation_channels_all:
        comments.append(
            "The number of cameras exceeds the number of available channels."
        )
    if no_of_cameras > max_cameras_in_installation_channels_ce:
        comments.append(
            "The number of cameras exceeds the number of available CE channels."
        )
        if data_dict["no_of_channels"] > data_dict["no_of_ce_channels"]:
            comments.append(
                "There are available non-CE channels. Check which channels you can legally use at the installation site."
            )
    ...
    if (
        data_dict["no_of_receivers"]
        < data_dict["no_of_transmitters"] // data_dict["max_tx_to_rx"]
    ):
    ...
    average_available_bitrate = data_dict["max_bitrate"] / (
        data_dict["max_distance_between_tx_and_rx"] / 100
    )
    total_available_bitrate = average_available_bitrate * data_dict["no_of_receivers"]
